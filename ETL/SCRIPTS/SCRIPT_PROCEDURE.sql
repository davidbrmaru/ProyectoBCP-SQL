...
USE [PROYECTOBCP]
GO
/****** Object:  StoredProcedure [dbo].[OBTENER_BASE_ACTIVO]    Script Date: 26/05/2022 21:26:27 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER  PROCEDURE [dbo].[OBTENER_ALL_BASE_ACTIVO]
...
BEGIN
		...
		WHERE D.COD_MATRICULA = @MATRICULA AND B.FLG_ACTIVO = 1 AND C.FLG_ACTIVO = 1 AND D.FLG_ACTIVO = 1 AND E.FLG_ACTIVO = 1 AND F.FLG_ACTIVO = 1 AND G.FLG_ACTIVO = 1
		ORDER BY A.ID DESC
END 
ELSE IF  @v_CL > 0 
		BEGIN
		...
		JOIN [dbo].[SQUAD] F ON E.ID_SQUAD = F.ID
		JOIN [dbo].[TRIBE] G ON F.ID_TRIBE = G.ID
		WHERE C.COD_MATRICULA = @MATRICULA AND B.FLG_ACTIVO = 1 AND C.FLG_ACTIVO = 1 AND D.FLG_ACTIVO = 1 AND E.FLG_ACTIVO = 1 AND F.FLG_ACTIVO = 1 AND G.FLG_ACTIVO = 1
		ORDER BY A.ID DESC
		END 
ELSE 
		BEGIN
		...
		JOIN [dbo].[SQUAD] F ON E.ID_SQUAD = F.ID
		JOIN [dbo].[TRIBE] G ON F.ID_TRIBE = G.ID
		WHERE  B.FLG_ACTIVO = 1 AND C.FLG_ACTIVO = 1 AND D.FLG_ACTIVO = 1 AND E.FLG_ACTIVO = 1 AND F.FLG_ACTIVO = 1 AND G.FLG_ACTIVO = 1
		ORDER BY A.ID DESC
		END
